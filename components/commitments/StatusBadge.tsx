...
const STATUS_CONFIG: Record<CommitmentStatus, { label: string; className: string; icon?: string }> = {
  to_check:      { label: 'To Check',      className: 'bg-blue-500/15 text-blue-600 dark:text-blue-400 border-blue-500/30' },
  expired:       { label: 'Expired',       className: 'bg-red-500/15 text-red-600 dark:text-red-400 border-red-500/30', icon: '⚠️' },
...
export default function StatusBadge({ status }: { status: CommitmentStatus }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-sm border ${className}`}>
      {icon && <span>{icon}</span>}
      {label}
    </span>
  )
